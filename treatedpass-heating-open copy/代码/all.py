...
DATA_DIR = "/root/data/treatedpass-heating-open/源数据"
...
# 每个处理组城市按“政策前协变量相似度”保留的 donor 数
TOP_K_DONORS = 20
...
def build_matrices(df: pd.DataFrame, treated_fips: int, t0: int):
    donor_fips_all = sorted([int(x) for x in df["fips"].dropna().unique() if int(x) != treated_fips])

    # 按“政策前协变量均值”选择与 treated 最相似的 TOP_K_DONORS 个 donor
    pre_df = df.loc[df["year_week"] < int(t0)].copy()
    if pre_df.empty:
        raise ValueError("政策前样本为空，无法按协变量筛 donor。")

    treat_cov_pre = (
        pre_df.loc[pre_df["fips"] == treated_fips, COV_COLS]
        .mean()
        .to_frame()
        .T
    )
    donor_cov_pre = (
        pre_df.loc[pre_df["fips"].isin(donor_fips_all), ["fips", "city"] + COV_COLS]
        .groupby("fips", as_index=False)
        .agg({"city": "first", **{c: "mean" for c in COV_COLS}})
    )
    donor_cov_pre = donor_cov_pre.dropna(subset=COV_COLS)
    if donor_cov_pre.empty:
        raise ValueError("donor 协变量均值为空，无法进行相似度筛选。")

    scaler_cov = StandardScaler()
    donor_cov_scaled = scaler_cov.fit_transform(donor_cov_pre[COV_COLS].values)
    treat_cov_scaled = scaler_cov.transform(treat_cov_pre[COV_COLS].values)[0]
    dists = np.sqrt(((donor_cov_scaled - treat_cov_scaled) ** 2).sum(axis=1))

    donor_cov_pre = donor_cov_pre.copy()
    donor_cov_pre["cov_distance"] = dists
    donor_cov_pre = donor_cov_pre.sort_values("cov_distance").reset_index(drop=True)

    top_k = int(TOP_K_DONORS) if int(TOP_K_DONORS) > 0 else len(donor_cov_pre)
    donor_selected = donor_cov_pre.head(min(top_k, len(donor_cov_pre))).copy()
    donor_fips = donor_selected["fips"].astype(int).tolist()
    ...
    meta = {
        ...
        "donor_selected_table": donor_selected,
        "donor_all_table": donor_cov_pre,
...
def run_one_city_file(data_path: str):
    ...
    donor_selected_table = meta.get("donor_selected_table")
    common_times = meta["common_times"]
    pre_times = meta["pre_times"]

    safe_city = sanitize_name(treated_city)

    if isinstance(donor_selected_table, pd.DataFrame):
        donor_csv = os.path.join(OUT_DIR, f"donor_top{TOP_K_DONORS}_by_cov_similarity.csv")
        donor_selected_table.to_csv(donor_csv, index=False, encoding="utf-8-sig")
        print(f"已保存：{donor_csv}")

...
# 0) 你只需要改这里（✅ 单文件：只跑驻马店）
# =========================
DATA_PATH = "/root/data/no-heating open/源数据/驻马店_panel_vs_controls_weekly_2018_2022_with_year_week_fips_with_cov.csv"

# 源数据目录（自动推导）
DATA_DIR = os.path.dirname(DATA_PATH)

# 输出基目录：与“源数据”同级（即 /root/data/no-heating open）
...
# =========================
# 4) GAN（仅用于扩充 donor）
#     参数保持：epochs=1500, batch_size=32, z_dim=64, lr=1e-4
# =========================
...
def main():
    ...
    print(f"[Single] 仅运行：{DATA_PATH}")
    print(f"[Single] 输出目录基准：{OUT_BASE_DIR}/驻马店/（按文件名自动建）")
    print(f"[Single] GAN 开启，倍率={GAN_RATIOS}")

    rows = run_one_city_file(DATA_PATH)  # 返回5行
    summary_df = pd.DataFrame(rows)

    summary_csv = os.path.join(OUT_BASE_DIR, "deepSCM_single_city_GAN_ratios_summary.csv")
    summary_df.to_csv(summary_csv, index=False, encoding="utf-8-sig")
    print(f"\n[Single] 5个倍率汇总已保存：{summary_csv}")
